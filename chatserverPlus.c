...
int main(){
	...
	FD_SET(sockfd,&allset);	//将侦听套接字放入集合
	maxfd = sockfd;
				...
				if(tmp_i == sockfd)
				{
					...
					if(recvbuf[0]=='/')
					{
						...
						if(strcmp(recvbuf,"/who\n")==0){
							...
							{
								if(FD_ISSET(tmpfd,&allset) && clientinfo[tmpfd].status == 0)	
                        ...
                        //hide实现隐身功能
						if(strcmp(recvbuf,"/hide\n")==0){
                            //修改该用户结构体
                            clientinfo[tmp_i].status = 1;
							continue;
						}

                        //onhide实现在线功能
						if(strcmp(recvbuf,"/unhide\n")==0){
                            //修改该用户结构体
                            clientinfo[tmp_i].status = 0;
							continue;
						}
